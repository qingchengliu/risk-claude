...
1. **选择分析后端**：
   ...
   - Question: "选择 codeagent 深度分析使用的后端："
   - Options:
     - "codex（推荐，精准但较慢）"
     - "claude（快速，适用于简单场景）"
   - 记录用户选择的后端名称（codex 或 claude）

2. **调用 codeagent 进行深度分析**：
   - **必须**调用 codeagent Skill 进行分析
   - codeagent 会根据 `--backend` 参数选择底层 AI 模型
   - 分析内容包括：
     - **代码探索**：理解项目结构和代码组织
   ...
   **codeagent 调用步骤**：
   1. 首先调用 `Skill("codeagent")` 激活 codeagent 技能
   2. 然后使用 Bash 工具执行 codeagent-wrapper 命令：

   ```bash
   codeagent-wrapper --backend <用户选择: codex 或 claude> - <<'EOF'
   任务：对以下需求进行深度技术分析
   ...
   **错误处理 - 退化机制**：
   - 若 codeagent 首次调用失败，重试一次
   - 若连续两次失败，才退化使用 Claude 主进程（Task 工具 + Explore agent）进行分析
   - 退化时需告知用户："codeagent 不可用，将使用 Claude 主进程进行分析"
...
- **需求不清**：追加提问轮次（最多 3 轮）
- **codeagent 失败**：重试一次，若连续两次失败则退化使用 Claude 主进程分析
- **方案分歧**：列出选项请用户决策
- **任务冲突**：调整文件范围或串行化执行
